...
//    fun <T>getNonPrimitiveData(key: String): T{
//        val soupString = preferences.getString(key, "")
//        val type = object: TypeToken<T>(){}.type
//        return Gson().fromJson(soupString, type)
//    }

    fun <T>getNonPrimitiveData(type: Type, key: String): T{
        val soupString = preferences.getString(key, "")
        //val typee = object: TypeToken<type>(){}.type
        return Gson().fromJson(soupString, type)
    }
    ...
    fun <T>putNonPrimitiveData(key: String, value: T){
        val editor = preferences.edit()
        val soupString = Gson().toJson(value)
        editor.putString(key, soupString)
        editor.apply()
    }
